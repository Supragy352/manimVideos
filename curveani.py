...
class curve(Scene):
    def construct(self):
        ...
        question_text = Text("Trace the Curve : a^2 * y^3 = x^2 (2a-x)*(x-a)").scale(0.6)
        ...
        lowest = Text("Equating the lowest degree term to Zero And Differentiating").scale(0.5).next_to(tangent, DR)
        equ = Text("(a^2 * y^2) + (2 * a^2 * x^2)= 0").scale(0.5).next_to(lowest, DOWN)
        equ2 = Text("y^2 = -2*x^2").scale(0.5).next_to(equ, DOWN)
        equ3 = Text("Therefore y is imaginary and there exist no tangent at (0,0)").scale(0.5).next_to(equ2, DOWN)
        ...
        asym_yaxis = Text("||el to y-axis : coefficient of highest degree term of y equal to Zero").scale(0.4).next_to(asymp_x, DR)
        ...
        curve_exists = Text("When x<a & x>2a curve does not exists").scale(0.4).next_to(region, DOWN)
        ...
        self.play(Create(intersection_point),Write(intersection_label))
        intersection_point1 = Dot(color=RED)
        intersection_label1 = Tex("(a, 0)").scale(0.4).next_to(intersection_point1, DL)
        self.play(Create(intersection_point1),Write(intersection_label1))
        intersection_point = Dot(color=RED)
        intersection_label2 = Tex("(0, 0)").scale(0.4).next_to(intersection_point2, DL)
        self.play(Create(intersection_point2),Write(intersection_label2))
